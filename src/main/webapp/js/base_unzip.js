...
var on = function ($, B, D) {
    var _ = [], B = B.replace(/^on/, "").toLowerCase();
    _.push($);
...
var format = function (_, B) {
    if (arguments.length > 1) {
        ...
        if (typeof(B) == "object")return _.replace(E, function (_, A) {
            ...
            var D = Array.prototype.slice.call(arguments, 1), $ = D.length;
    ...
    return _
}, isType = {types: ["Array", "RegExp", "Date", "Number", "String", "Object"]};
...
var cache = (function () {
    ...
    return {get: B, set: C, is: _, remove: A}
})(), Browser = (function () {
...
function getHost(A) {
    var _ = A || location.host, $ = _.indexOf(":");
    return ($ == -1) ? _ : _.substring(0, $)
}
window.baidu = window.baidu || {version: "1-0-0"};
baidu.sio = baidu.sio || {};
baidu.url = baidu.url || {};
...
baidu.sio.get = function (E, D) {
    ...
    var D = D || function () {
        }, B = baidu.url.parseQuery(E, "callback"), _ = document.createElement("SCRIPT");
    if (!B) {
        var $ = false;
...
-function () {
    var $ = new (function () {
        var D = {}.hasOwnProperty ? true : false, _ = function ($) {
            return $ < 10 ? "0" + $ : $
        }, C = {
            "\b": "\\b",
            "\t": "\\t",
        ...
        this.encode = function (F) {
            if (typeof F == "undefined")return "undefined"; else if (F === null)return "null"; else if (F instanceof Array)return E(F); else if (F instanceof Date)return A(F); else if (typeof F == "string")return B(F); else if (typeof F == "number")return isFinite(F) ? String(F) : "null"; else if (typeof F == "boolean")return String(F); else {
                var C = ["{"], _, G, $;
...
var Config = {FormConfig: {}, UserTrackConfig: {}};
var ieUserData = {
    init: function () {
        ...
        if ($.type == "link") {
            UserTrack.send({level: "1", page: "index", type: "mail", index: _.selectedIndex});
            window.open($.action);
            _.selectedIndex = 16
        }
    }
    ...
    var J = {}, B = [], A = null, I = [], E = null;

    function F(_, A) {
        var D = C("input"), B = C("input"), $ = C("input");
        D.type = B.type = $.type = "hidden";
        D.name = "oq", B.name = "rsp", $.name = "f";
        D.value = A;
        B.value = _ - 1;
        if (document.f.action == "http://zhidao.baidu.com/q")$.value = "sug"; else $.value = "3";
        I.push(D);
        I.push(B);
        I.push($);
    ...
    function _() {
        ...
        var C = cache.get("CURRENT_TAB_SCOURCE"), _ = G("q").value;
        if (B[0])B[0].value = _;
        document.f.submit();
        try {
            ...
            if (byId("content").offsetWidth == 990)UserTrack.send({
                ...
                page: "index",
                type: "form"
            }); else UserTrack.send({level: 1, page: "indexk", type: "form"})
        }
    ...
    function H(I, K, _) {
        var K = K, H = cache.set("CURRENT_TAB_SCOURCE", K), F = Config.FormConfig;
        $ = K;
        G("q").value = "";
        G("q").focus();
        function E() {
        ...
        function L() {
            if (J.key)J.key.className = "tab_item"; else G("wegTab").className = "tab_item";
            I.className = "tab_cur";
            J.key = I
        }
    ...
    return {tab: H, submit: _, sugCurIndex: A, oq: E}
})();
function bottomForm() {
    var $ = document.bf.key.value;
    if (document.bf.sto[0].checked)if (byId("content").offsetWidth == 990)window.open("http://www.baidu.com/s?tn=hao123&word=" + $, "mspg6"); else window.open("http://www.baidu.com/s?tn=sitehao123_1_pg&word=" + $, "mspg6");
...
function morePopup(_, A) {
    var _ = _ || window.event, B = _.target || _.srcElement, C = G("hao_more_event"), $ = G("hao_more_popup");
    switch (A) {
        case"down":
            if ($.style.display == "") {
            ...
            if (_.stopPropagation)_.stopPropagation(); else _.cancelBubble = true;
            break;
        case"over":
...
function byTag($) {
    ...
    var A = 1, S = G("sug"), B = G("q"), N, Z = null, V = -1, E = false, Q = false, $ = false, F = true, R = null, H = null, Y = 0;
    ...
    function P($) {
        ...
        if ((new Date() - O.startTime) / 1000 > 0.5 && L != _.innerHTML)return; else {
            DynamicForm.submit();
            B.value = L;
            c()
        }
